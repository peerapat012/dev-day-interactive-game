...
interface SummaryContentProps {
  onStartNewRound?: () => Promise<void> | void;
  startNewRoundDisabled?: boolean;
  startNewRoundLabel?: string;
  startNewRoundHelp?: string;
}

export function SummaryContent({
  onStartNewRound,
  startNewRoundDisabled,
  startNewRoundLabel = "Start new round",
  startNewRoundHelp = "Clears all guest phrases and groups so a new session can begin.",
}: SummaryContentProps = {}) {
  const {
    topGroups,
    summaries,
  ...
  const showNewRoundCta = Boolean(onStartNewRound && hasSummarized);
    ...
    <div className="flex flex-col gap-6">
      ...
      {!loading && !hasSummarized && entryCount > 0 && !error ? (
        <StatusPanel>
          ...
          all inputs and show the top groups.
        </StatusPanel>
      ) : null}
      ...
      {showNewRoundCta ? (
        <div className="rounded-3xl border border-white/10 bg-zinc-900/70 p-5">
          <p className="text-base font-semibold text-zinc-100">
            Ready for a fresh round?
          </p>
          <p className="mt-1 text-sm text-zinc-400">{startNewRoundHelp}</p>
          <Button
            type="button"
            onClick={() => onStartNewRound && onStartNewRound()}
            disabled={startNewRoundDisabled}
            className="mt-4 w-full sm:w-auto"
          >
            {startNewRoundDisabled ? "Resetting…" : startNewRoundLabel}
          </Button>
        </div>
      ) : null}
    </div>
